...
class UserForm(forms.Form):
	...
	def clean(self):
		...
		if user:
			# 如果已经注册过
			raise forms.ValidationError({'username': '用户已经存在，请直接登录'})
		else:
			# 没有注册过
			pass
		# 校验密码和确认密码是否相同
		if self.cleaned_data.get('password') != self.cleaned_data.get('password2'):
			raise forms.ValidationError({'password': '两次密码不相等'})
...
class UserLoginForm(forms.Form):
	...
	username = forms.CharField(
		required=True, max_length=5, min_length=2,
		error_messages={
			'required': '用户名必填',
			'max_length': '用户名不能超过5个字符',
			'min_length': '用户名不能少于2个字符'
	...
	def clean(self):
		...
		user = User.objects.filter(username=self.cleaned_data.get('username'))
		if not user:
			raise forms.ValidationError({'username': '请先注册再登录'})
		return self.cleaned_data
