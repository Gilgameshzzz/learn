...
if __name__ == '__main__':
    ...
    注意：1、其中0-1024是‘著名端口’用来绑定一些特殊服务的，一般不使用
         2、同一时间一个端口只能绑定一个服务
    """
    server.bind(('10.7.181.83', 8080))

    # 3、监听
    """
    参数：用来限制一次性能够接受的客户端请求的数量
    """
    server.listen(20)
    print('开始监听请求')
    ...
    while True:
        ...
        字符串.encode(encoding = 编码方式，默认为utf-8)
        """
        str3 = input('>>>')
        # client.send('HTTP/1.1 200 OK\r\n\r\n'.encode(encoding='utf-8'))
        # client.send(bytes('你哈啊', encoding='utf-8'))
        client.send(str3.encode())
